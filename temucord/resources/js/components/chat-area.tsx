...
export function ChatArea({ channel }: { channel: any }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        axios.post(`/channels/${channel.id}/posts`, { content: messageToSend })
            .then(res => {
    ...
    return (
        <div className="flex flex-col h-full bg-[#313338] text-gray-200">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {channel.description && (
                        <>
                            ...
                            <span className="text-sm text-gray-400 truncate">{channel.description}</span>
                        </>
                    )}
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
                {loading ? (
                    <div className="text-center text-gray-400 mt-4">Cargando mensajes...</div>
                ) : posts.length === 0 ? (
                    <div className="flex flex-col items-start justify-end h-full text-gray-400 pb-4">
                        ...
                        <h3 className="text-3xl font-bold text-white mb-2">¡Te damos la bienvenida a #{channel.name}!</h3>
                        <p>Este es el comienzo del canal.</p>
                    </div>
                ) : (
                    posts.map((post: any) => (
                        <div key={post.id} className="flex gap-4 hover:bg-[#2e3035] p-2 -mx-2 rounded transition">
            ...
            <div className="p-4 shrink-0">
                <form onSubmit={handleSubmit} className="relative flex items-center bg-[#383a40] rounded-lg">
                    <input
                        type="text"
                        value={content}
