...
export function ServerSidebar({ server }: { server: any }) {
    // Si no hay servidor seleccionado, el panel no se muestra
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-4 custom-scrollbar">
                ...
                <div>
                    ...
                    <div className="space-y-0.5 mt-2">
                        {/* MAPEO REAL DE LA BASE DE DATOS */}
                        {server.channels && server.channels.length > 0 ? (
                            server.channels.map((channel: any) => (
                                <button key={channel.id} className="group flex w-full items-center justify-between rounded px-2 py-1.5 text-gray-400 hover:bg-[#35373c] hover:text-gray-300 transition">
                                    <div className="flex items-center gap-2">
                                        {/* Icono dinámico según tu base de datos (open/closed) */}
                                        {channel.type === 'closed' ? (
                                            <Lock size={20} className="text-gray-500 group-hover:text-gray-400" />
                                        ) : (
                                            <Hash size={20} className="text-gray-500 group-hover:text-gray-400" />
                                        )}
